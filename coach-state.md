# COACH STATE — live training status
*Maintained by the coach. Last updated: September 7, 2026 (after the Aug 29 – Sep 7 backlog was logged)*

**Read this file at the start of every session, together with `workout-log.json`.** It holds current working weights, active decisions, and open questions. Where it disagrees with the skill's reference files, THIS FILE WINS — the reference files (`brandon-profile.md`, `exercise-library.md`) hold slow-changing background (meals, injury history, preferences) and their weight tables are stale as of June. This file holds the live state.

---

## Where the program is right now

**THE STRENGTH BLOCK IS OVER. He is already training hypertrophy — make it official.**

The block began Jul 14 at 4×5–6 with 2–3 min rest, and was planned to end in late August. It is now September and Brandon has drifted into 8–12 reps on his own across every recent session: bench 175×10 then 185×9/9, squat 205 for 8/8/10, leg extension 12s, Hammer Strength machines at 12s. He was not told to do this. He found the rep range his body wanted.

**Program it as a hypertrophy block from here:** main lifts 3–4 × 8–10, accessories 3 × 10–15, 2 min rest on compounds and 90 sec on accessories (which is already what he does). Revisit heavy 5s in late October at the Costa Rica home gym, which has a squat rack.

---

## RESOLVED — right scapular strain (Aug 21 → cleared Sep 6)

**What happened:** Aug 21, hotel gym, limited sleep. Strained the right mid-back at the inferior border of the shoulder blade while hoisting a 50 lb dumbbell to his knee to set up for a press. **The setup, not the lift.** Aborted the session at 20 min — correct call.

**This was a RECURRENCE of the June 9 rhomboid/mid-trap strain.** Same site, same mechanism (loaded DB setup), ~10 weeks apart.

**Cleared on the evidence, not the calendar.** Between Aug 30 and Sep 7 he completed, all pain-free: chest-supported rows, front pulldowns, cable rows at 145, lat pulldowns, **hanging** knee raises, skull crushers (previously restricted), and hoisting 60 lb dumbbells into a press — the exact movement that caused it.

**Restrictions lifted:** skull crushers, T-bar row, KB bent-over row, heavy DB hoisting, hanging work. All back on the table.

**Standing rules that DO NOT expire:**
- **Every dumbbell comes off a rack at hip height or higher. Never hoisted from the floor to the knee.** This is the mechanism that has now injured him twice.
- Thoracic mobility twice weekly, indefinitely — thread-the-needle, open books, foam roll upper back. He started doing thread-the-needle on his own in Boston; reinforce it.
- **Two strains at the same site in ten weeks is a pattern.** A physical therapist assessment was recommended Aug 24 and has not happened. Raise it again if there is any third episode, and it is worth doing regardless.

## RESOLVED — lower back (was the Aug 10 active issue)

The Aug 3–8 lower back complaints stopped once the hinge was periodized down. He has run **Smith RDL at 165 total for 3×10** on Aug 29 and Sep 6 with zero back complaints, unstrapped. The earlier problem was the load curve (205 → 235 in six days), not the movement. **Keep the hinge in the 8–12 rep range at moderate load. Do not chase heavy fives on the RDL again.**

---

## Current working weights (as of Sep 7, 2026)

### Upper
| Lift | Current | Next step |
|------|---------|-----------|
| **Barbell Bench** | 175 × 10, then **185 × 9, 9** (Sep 7) | Big jump — Aug 2 the same 185 gave 5/5/5/4. Run 185 for 3×10; when all 30 reps land, go 195. |
| **DB Shoulder Press** (LEAD LIFT) | 55 × 12, **60 × 10** (Sep 7) | 60 is real, and it came LATE in the session because benches were taken. Program 60 for 3×10 in the lead slot. Stays first, permanently. |
| **Seated Cable Row** | 145 × 10/10/9 (Sep 7) | Was 170 pre-injury. Climb back deliberately: 155 next, then 165. |
| **Lat Pulldown** | 120 × 10/9 (Sep 7, straight bar wide grip) | Was 150 pre-injury. Go 130, then 140. Prefers the neutral-grip bar when available. |
| **Pull-ups** | **4–5 strict** (Aug 6, hotel: 4 pronated / 5 neutral) | Untested since. The Sep 6 "12 reps" was a dictation error — that was Pallof press. Retest before programming. |
| Face Pulls | Ropes unavailable Sep 7; subbed reverse cable fly 15 lb/arm × 12/10/9 | Resume face pulls at 42.5 (functional trainer) when ropes exist |
| Lateral Raise | 15 lb × 15 × 3 | Consistent |
| Incline DB Curl | 25 lb × 12 × 3 | Up from 20 lb |
| Hammer Curl | 25 lb × 10 × 3 | |
| Skull Crusher | 40 lb curl bar × 12 × 3 | **CLEARED** — pain-free Aug 30 |
| Rope Pushdown | Machine-dependent: 30–47.5 lb | Log the machine, not just the number |

### Lower
| Lift | Current | Next step |
|------|---------|-----------|
| **Smith Squat** | **215 × 6×4** (Aug 29) · **205 × 8/8/10** (Sep 6) | Sep 6's best set was his LAST — he left reps there. Run 215 for 3×8. |
| **Smith RDL** | 165 total (70/side) × 10 × 3, no straps | Holding here is correct. Hamstring stretch focus, moderate load, high rep. |
| **Seated Leg Curl** | **70 lb — STALLED, this is the problem lift** | Earned 80 on Aug 16 (clean 12/12/12) and has run 70 every session since: 12/10/10, then 12/11. **Go to 80. Do not program 70 again.** |
| Leg Extension | 85 × 12, 90 × 10/10 (Sep 6) | 90 for 3×12 |
| Walking Lunge (DB) | 30 lb × 10/leg × 3 (Sep 6) | Back to 35 lb |
| Hip Thrust Machine | 200 lb × 3×12 | Not used since August — reintroduce when a machine is available |
| Glute Bridge Machine | 115 × 12 (Aug 29) | **He dislikes it** — the strap digs into his hip. Use only if no hip thrust machine. |

### Hammer Strength baselines (travel gyms, PER ARM)
Established Aug 30 in Boston. Use these instead of guessing when he hits a PF with Hammer Strength.
| Machine | Working weight |
|---------|----------------|
| Shoulder Press | 80/arm × 12 |
| Chest Press | 80/arm × 8–9 |
| Chest-Supported Row | 60/arm × 10 |
| Front Pulldown | 70/arm × 12 |

### Core (rotate, never repeat a circuit back to back)
Pallof Press 22.5 lb × 12/side · KB Plank Drag **30 lb** × 20 (20 felt light) · Dead Bugs 20–24 · Side Plank 45 sec/side · Plank Shoulder Tap 20 · Hanging or machine Knee Raise 10–12 · Bird Dog 16

### Rest periods — he standardized these himself and flagged them for data
2 min on main compounds, 90 sec on accessories, 45 sec between core rounds.

---

## Nutrition — currently dark, and that is the biggest gap in the program

**`fitness-data.json` has not updated since May 17, 2026.** No calories, protein, sleep, weight, or deficit data for nearly four months. All coaching since then has been training-only. Either the app stopped saving that file or the save is broken — worth diagnosing.

**Aug 25 – Sep 5 (vacation/travel), self-reported:** ate poorly, more alcohol than typical, ~7k steps/day average, light tennis on some days, one cardio session. No lifting for roughly ten days. He came back and immediately hit PRs on bench and shoulder press, so the layoff did not cost him — if anything the rest helped.

**Targets on file (unverified against reality):** 1,900 kcal/day, 140 g protein, TDEE ~2,200–2,500. Goal is body recomposition with no deadline; scale is secondary to strength and composition.

---

## Equipment reality — read before programming any weight

Brandon has trained at **seven-plus gyms since mid-July**: multiple Planet Fitness locations, PF Boston, PF Eugene, several hotel gyms, one PF with a squat rack.

- **Smith bars vary and are often unmarked.** ~24–25 lb estimated. Two machines with identical plates can differ 10–20 lb in effective resistance. **Program and log PLATES PER SIDE.**
- **Functional trainer stacks run a different ratio than standard stacks.** A 60 lb face pull on a standard stack is 37.5–42.5 on a functional trainer. Not a strength change.
- **Cable increments vary** — some 120→135→150, some 160→170 with no 5 lb step.
- **Hotel gyms:** DBs only, usually capped at 75 lb, KBs at 30 lb, no barbell, no Smith.
- **Always write a named fallback into every exercise.** He has been shut out of the leg curl machine twice and had benches occupied on Sep 7.
- **Always log which gym.**

### Free-weight transition status
- **Bench: TRANSITIONED.** Free barbell since Aug 2, on his own initiative. Carryover from the Smith was near-total.
- **Squat: NOT transitioned.** Still Smith. Free squatting is a different skill. October on-ramp at the Costa Rica home gym: Wk1 135 × 4×6, Wk2 155, Wk3 175. Safety pins below depth on every set — he trains alone.
- **Floor deadlifts remain off.**

---

## Training schedule (set by Brandon, Sep 7)

**4 lifting sessions per 7 days, maximum spacing.** Current rotation: **Lower Thursday, Upper Saturday**, with the prior week's Sun/Mon pair. Cardio or rest on the off days.

This gives ~96 hr between lower sessions and ~120 hr between upper sessions. It is more rest than a standard 2x/week split, and it suits him — he came back from a 10-day layoff and immediately PR'd bench and shoulder press. **The trade-off: each session carries more volume.** Program 6 movements plus core on lower days rather than 5, and mark the last accessory as droppable if the session passes 70 min.

---

## Open questions
- **Where is he training now, and for how long?** The 2-month settled location determines whether we can run a consistent progression instead of re-baselining every week.
- **Aug 15 session is still missing** from the log (a Saturday session he cut short).
- **The Aug 25/26 cardio date is uncertain** — programmed for Tue Aug 25, he later referred to it as Wednesday. Logged under Aug 25.
- Is the nutrition file fixable, or does tracking need a different home?
- He mentioned "feeling agitated" on Sep 7 and skipped his finisher for it. Not a training problem on its own; watch whether it recurs.

## How Brandon works
- Wants **specific weights every time.** Never "moderate" or "as tolerated."
- **Ramp-up sets are a proven lever.** When he ramps he hits his numbers; when he skips them he opens light and reports feeling weak. Write the ramp explicitly into the warmup — he has said outright that if it is not written down, he does not do it.
- **Excellent in-session judgment.** Drops weight, slows tempo, resequences around occupied equipment, and stops sets at the right moment instead of grinding. Trust it and log what he actually did.
- **He will correct you, and he is usually right.** He caught a lower-day scheduling error and a misdiagnosed injury site in the same week. Take the correction, fix the record, and say so plainly.
- Will sometimes exceed the programmed weight on his own. Good instinct — ask him to flag when he is feeling strong so the bigger number gets written in rather than tested cold.
- **He dictates his logs with Wispr Flow, and it produces real transcription errors** — "265 lb dumbbells" meant 65, "1:40" meant 140, and "pull-ups, 12 reps each side" was actually Pallof press. **Errors substitute whole exercise names, not just numbers.** When anything is self-contradictory or out of character for his current level, flag it and ask rather than guessing — he corrects it immediately.
- **He sometimes re-pastes the same note twice.** Aug 15 and Aug 8 arrived as identical text. Never log a session you cannot distinguish from one already recorded — a phantom entry is worse than a missing one.
